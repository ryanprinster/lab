...
class ActorCriticNetwork:
    def __init__(self, name, num_envs=4, n=20, entropy_reg_term=.05, 
        ...
        with tf.variable_scope(name):
            ...
            # Initiated naively to zero state. R2D2 paper shows this strategy to be naive.
            ...
            # Used for in loss  calculation (trfl)
...
class A2CAgent(object):
    def __init__(self, hyperparams, level, tensorboard_path, train_episodes):
        ...
        self.ac_network = ActorCriticNetwork(name='main_acn', \
            num_envs=self.hp['num_envs'], n=self.n)
    ...
    def train(self):
        ...
        print("num_envs", self.hp['num_envs'])
        ...
        with tf.Session() as sess:
            ...
            action_list = np.random.randint(6, size=self.hp['num_envs'])
            reward_list = np.zeros(self.hp['num_envs'])

            for ep in range(1, self.train_episodes):
                print("Episode: ", ep)

                # n-steps
                n_step_rewards = np.zeros((self.hp['n'], self.hp['num_envs']))
                n_step_actions = np.zeros((self.hp['n'], self.hp['num_envs']))
                n_step_next_states = np.zeros((self.hp['n'], self.hp['num_envs'], 
...
def run(level, tensorboard_path, num_envs=None, n=None, max_steps=None, 
    learning_rate=None):

    # Enable commandline hyperparameter specification
    hyperparams = {}
    if num_envs:
  ...
  agent = A2CAgent(hyperparams, level=level, num_envs=4, n=10, 
...
if __name__ == '__main__':
  ...
  run(args.level_script, args.tensorboard_path, args.num_envs, args.max_steps, 
    args.n, args.learning_rate)
